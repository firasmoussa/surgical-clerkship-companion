  ...
  return (
    <div className="py-8 sm:py-12">
      <section aria-labelledby="welcome-heading" className="border-b border-border-warm pb-8 sm:pb-10">
        ...
        <h1 id="welcome-heading" className="max-w-2xl font-serif italic text-[32px] sm:text-[42px] font-normal leading-tight text-ink">
          A clearer picture.<br />A more prepared you.
        </h1>
        <p className="mt-5 max-w-xl text-[14px] text-secondary leading-relaxed">
        ...
        <a href="#procedures" className="mt-6 inline-flex items-center gap-3 rounded-[6px] bg-ochre px-4 py-2.5 text-[13px] font-medium text-parchment transition-colors hover:bg-charcoal focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ochre">
      ...
      <section id="procedures" aria-labelledby="procedures-heading" className="scroll-mt-6 pt-8">
        <div className="mb-5 flex flex-wrap items-baseline justify-between gap-2">
          <h2 id="procedures-heading" className="font-serif italic text-[20px] font-normal text-ink">Choose a procedure</h2>
          <span className="text-[11px] text-muted uppercase tracking-wider">Two guides, one place to prepare</span>
        </div>
        <div className="grid gap-5 md:grid-cols-2">
          {procedures.map((procedure) => (
            <article key={procedure.href} className="flex flex-col rounded-lg border border-border-warm bg-surface p-5 sm:p-6">
              <div className="mb-5 flex items-center justify-between gap-4">
                <span className="text-[11px] uppercase tracking-wider text-secondary">{procedure.shortName}</span>
                <span aria-hidden="true" className="font-serif italic text-[28px] font-normal leading-none text-ochre">{procedure.number}</span>
              </div>
              <h3 className="max-w-xs font-serif italic text-[24px] font-normal leading-tight text-ink">{procedure.name}</h3>
              <p className="mt-3 text-[13px] leading-relaxed text-secondary">{procedure.description}</p>
      ...
      <section aria-labelledby="review-heading" className="mt-10 border-t border-border-warm pt-8">
        <h2 id="review-heading" className="font-serif italic text-[20px] font-normal text-ink">Make the most of your review</h2>
          ...
          {reviewSteps.map((step, index) => (
            <li key={step.title} className="flex gap-3">
              ...
              <div className="pt-0.5">
                ...
                <p className="mt-1 text-[13px] text-secondary leading-relaxed">{step.body}</p>
              </div>
            </li>
          ))}
        </ol>
      ...
      <div className="mt-10 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border-warm p-4">
        <p className="text-[13px] text-secondary">Heard a good question in the OR? Help the next student prepare.</p>
        <Link href="/submit" className="text-[13px] text-ochre underline underline-offset-4 focus-visible:outline-2 focus-visible:outline-ochre">Contribute a question <span aria-hidden="true">→</span></Link>
      </div>
    </div>
  );
}
