...
type FailureStage = "configuration" | "client_address" | "rate_limit_timeout" | "rate_limit_service" | "database_insert" | "database_client";
const unavailable = (stage: FailureStage) => {
  // Log only fixed codes. Never log errors, credentials, addresses, or form data.
  console.error("[submissions] unavailable", stage);
...
export async function POST(req: NextRequest) {
  ...
  if (!url || !key || !redisUrl || !redisToken || !salt) return unavailable("configuration");
  ...
  if (!ip || !isIP(ip)) return unavailable("client_address");
  try {
    ...
    if (result.reason === "timeout") return unavailable("rate_limit_timeout");
  ...
  } catch { return unavailable("rate_limit_service"); }
  ...
  try {
    ...
    if (error) return unavailable("database_insert");
    return NextResponse.json({ ok: true }, { status: 201 });
  } catch { return unavailable("database_client"); }
}
