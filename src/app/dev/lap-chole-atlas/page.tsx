  ...
  return (
    <section className="py-6">
      ...
      <div className="mt-5 mb-6">
        ...
        <h1 className="font-sans tracking-tight text-[30px] text-ink font-semibold">Laparoscopic cholecystectomy</h1>
        <p className="mt-2 text-[14px] text-secondary leading-relaxed">The appendectomy atlas style, applied to the biliary anatomy. Switch versions to compare the illustrations.</p>
      </div>
      <div className="mb-4 inline-flex overflow-hidden rounded-lg border border-border-warm" aria-label="Illustration version">
        {(["atlas", "current"] as const).map((value) => (
          <button key={value} type="button" aria-pressed={version === value} onClick={() => setVersion(value)} className={`px-3.5 py-1.5 text-[14px] transition-colors focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-ink ${version === value ? "bg-ochre text-parchment font-medium" : "text-secondary hover:bg-surface"}`}>
            {value === "atlas" ? "Atlas preview" : "Previous illustration"}
          </button>
        ))}
      </div>
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_240px]">
        <figure className="min-w-0 self-start rounded-2xl border border-border-warm bg-card p-4">
          ...
          <button type="button" aria-pressed={enlarged} onClick={() => setEnlarged((previous) => !previous)} className="mb-3 rounded-lg border border-border-warm px-3 py-1.5 text-[12px] text-secondary focus-visible:outline-2 focus-visible:outline-ochre sm:hidden">{enlarged ? "Fit illustration" : "Enlarge illustration"}</button>
        ...
        <aside className="rounded-2xl border border-border-warm bg-card p-4 self-start">
          ...
          <div aria-live="polite" aria-atomic="true" className="mt-4 rounded-md border-l-[3px] border-ochre bg-surface p-3">
            ...
            <p className="font-sans tracking-tight text-[16px] font-semibold text-ink">{selected ?? "All landmarks visible"}</p>
          </div>
